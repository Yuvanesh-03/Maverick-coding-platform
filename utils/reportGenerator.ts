...
export const generateAdminUsageReportPdf = (data: AdminReportData) => {
    ...
    try {
        ...
        console.error('Error generating Top Users table:', error);
        addText('Error generating Top Users table');
        y += 20;
    }
    ...
    try {
        ...
        console.error('Error generating Top Contributors table:', error);
        addText('Error generating Top Contributors table');
        y += 20;
    }
    ...
    try {
        ...
        console.error('Error generating Skill Distribution table:', error);
        addText('Error generating Skill Distribution table');
        y += 20;
    }
...
export const generateUserProfilePdf = (user: UserProfile) => {
    ...
    try {
        ...
        console.error('Error generating Key Statistics table:', error);
        doc.setFontSize(11);
        doc.text('Error generating Key Statistics table', 14, y);
        y += 20;
    }
    ...
    if (user.skills && user.skills.length > 0) {
        try {
            ...
            console.error('Error generating Skills table:', error);
            doc.setFontSize(11);
            doc.text('Error generating Skills table', 14, y);
            y += 20;
        }
    } else {
        doc.setFontSize(11);
    ...
    if (user.learningPaths && user.learningPaths.length > 0) {
        ...
        user.learningPaths.forEach(path => {
            ...
            try {
                ...
                console.error('Error generating Learning Path modules table:', error);
                doc.setFontSize(10);
                doc.text('Error generating modules table', 16, y);
                y += 20;
            }
        });
    }
    ...
    if (user.activity && user.activity.length > 0) {
        ...
        try {
            ...
            console.error('Error generating Activity History table:', error);
            doc.setFontSize(11);
            doc.text('Error generating Activity History table', 14, y);
            y += 20;
        }
    }
