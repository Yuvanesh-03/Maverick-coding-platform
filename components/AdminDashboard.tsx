...
const StatCard: React.FC<{ title: string, icon: React.ReactNode, children: React.ReactNode }> = ({ title, icon, children }) => {
    return (
        <div className="glass-effect rounded-[16px] p-4 border border-slate-200/20 dark:border-slate-700/30 hover:shadow-lg transition-shadow duration-300">
            <div className="flex items-center gap-3">
                <div className="bg-gradient-to-br from-purple-500/20 to-indigo-500/20 p-3 rounded-lg text-purple-400 dark:text-purple-300">
                    {icon}
                </div>
                <div>
                    <h4 className="text-sm font-medium text-slate-400 dark:text-slate-300">{title}</h4>
                    <div className="text-slate-100 dark:text-slate-100">{children}</div>
                </div>
            </div>
        </div>
    );
    ...
    const userManagementFilters = (
        <div className="flex flex-wrap items-center gap-2">
            <input
                ...
                className="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-1.5 text-sm text-gray-900 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent w-full sm:w-auto"
            />
            <select
                value={filterSkill}
                onChange={(e) => setFilterSkill(e.target.value)}
                className="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 text-sm text-gray-900 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
                {allSkills.map(skill => (
    ...
    return (
        <>
            ...
            <div className="space-y-8">
                ...
                <Card title="User Management" icon={<UserIcon className="h-5 w-5" />} headerActions={userManagementFilters} disable3D={true}>
                    {isLoading ? (
                ...
                <Card title="Platform Management" icon={<CogIcon className="h-5 w-5" />} disable3D={true}>
