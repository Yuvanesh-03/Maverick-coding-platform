...
import { generateEnhancedUserReport } from '../utils/enhancedReportGenerator';
...
const AdminUserProfileView: React.FC<AdminUserProfileViewProps> = ({ userId, onBack }) => {
    ...
    const handleGenerateEnhancedReport = () => {
        if (!profile) return;
        try {
            generateEnhancedUserReport(profile);
        } catch (error) {
            console.error("Failed to generate enhanced PDF report:", error);
            alert("An error occurred while generating the enhanced report.");
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 ...
                 <Card title="Manual Overrides" icon={<CogIcon/>}>
                      <div className="flex flex-wrap gap-2">
                         ...
                         <Button variant="secondary" onClick={handleGenerateReport}>Basic Report</Button>
                         <Button variant="primary" onClick={handleGenerateEnhancedReport}>✨ Enhanced Report</Button>
                      </div>
                 </Card>
            </div>
        </div>
