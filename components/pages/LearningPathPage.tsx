    ...
    return (
        <div className="space-y-6">
            <div className="flex space-x-1 rounded-lg bg-gray-100 dark:bg-gray-900/50 p-1">
                {tabs.map(tab => (
                    <button key={tab.id} onClick={() => setView(tab.id as LearningPathView)} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 text-base font-semibold ${view === tab.id ? 'bg-[#4299e6] text-white shadow-[0_2px_8px_rgba(0,0,0,0.08)]' : 'text-[#4a5568] dark:text-gray-300 hover:bg-white dark:hover:bg-gray-700/50'}`}>
                        {tab.icon} {tab.name}
                    </button>
                ))}
            </div>
