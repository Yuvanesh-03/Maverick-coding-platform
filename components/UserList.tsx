...
const skillLevelColor: Record<SkillLevel, string> = {
  'Expert': 'bg-teal-500/20 text-teal-600 dark:text-teal-300',
  'Advanced': 'bg-blue-500/20 text-blue-600 dark:text-blue-300',
  'Intermediate': 'bg-sky-500/20 text-sky-600 dark:text-sky-300',
...
const UserRow: React.FC<{ user: UserProfile; isExpanded: boolean; onToggle: () => void; onViewUser: (userId: string) => void; onDeleteUser: (userId: string, userName: string) => void; index: number; }> = ({ user, isExpanded, onToggle, onViewUser, onDeleteUser, index }) => {
    ...
    return (
        <>
            <tr onClick={onToggle} 
                className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-800/30' : 'bg-gray-50/50 dark:bg-gray-800/60'} hover:bg-blue-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer animate-fade-in-stagger`} 
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300 text-center animate-slide-up-fade">
