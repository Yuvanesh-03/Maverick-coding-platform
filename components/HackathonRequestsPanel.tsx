...
const HackathonRequestsPanel: React.FC<HackathonRequestsPanelProps> = ({ requests, onUpdateRequest, onRefresh }) => {
    ...
    const tabButtons = (
        <div className="flex space-x-1 rounded-lg bg-gray-100 dark:bg-gray-900/50 p-1">
            <button
                ...
                className={`flex-1 px-3 py-1.5 text-base font-semibold rounded-lg transition-all duration-200 ${activeTab === 'pending' ? 'bg-[#4299e6] text-white shadow-[0_2px_8px_rgba(0,0,0,0.08)]' : 'text-[#4a5568] dark:text-gray-300 hover:bg-white dark:hover:bg-gray-700/50'}`}
            >
                Pending ({pendingRequests.length})
            </button>
            <button
                onClick={() => setActiveTab('reviewed')}
                className={`flex-1 px-3 py-1.5 text-base font-semibold rounded-lg transition-all duration-200 ${activeTab === 'reviewed' ? 'bg-[#4299e6] text-white shadow-[0_2px_8px_rgba(0,0,0,0.08)]' : 'text-[#4a5568] dark:text-gray-300 hover:bg-white dark:hover:bg-gray-700/50'}`}
            >
    ...
    return (
        <>
            ...
            <Card title="Hackathon Requests" icon={<TrophyIcon className="h-5 w-5" />} headerActions={tabButtons} disable3D={true}>
