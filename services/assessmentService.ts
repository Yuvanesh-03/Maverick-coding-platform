...
export const getAssignedAssessmentsForUser = async (userId: string): Promise<CustomAssessment[]> => {
    ...
    try {
        const customAssessmentsRef = firestore.collection('customAssessments');
        const allUsersQuery = customAssessmentsRef.where('assignees.type', '==', 'ALL_USERS');
        
        const specificUserQuery = customAssessmentsRef 
