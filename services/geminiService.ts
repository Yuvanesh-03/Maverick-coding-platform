...
try {
  ...
  console.error("Gemini AI could not be initialized. API key is likely missing or not available in this environment.");
}

const geminiNotAvailableError = (functionName: string) => {
    const message = `Gemini service is not available, cannot call ${functionName}. Check API key configuration.`;
...
export const generateAssessmentQuestions = async (config: AssessmentConfig): Promise<MCQQuestion[]> => {
    if (!ai) {
        geminiNotAvailableError('generateAssessmentQuestions');
        return [{ type: QuestionType.MCQ, questionText: "Error: Could not generate questions.", options: ["AI service not available.", "", "", ""], correctAnswerIndex: 0 }];
    }
...
export const generateDynamicQuiz = async (config: QuizConfig): Promise<AssessmentQuestion[]> => {
    if (!ai) {
        geminiNotAvailableError('generateDynamicQuiz');
        return [{ type: QuestionType.MCQ, questionText: "Error: Could not generate questions.", options: ["AI service not available.", "", "", ""], correctAnswerIndex: 0 }];
    }
...
export const evaluateAssessment = async (questions: AssessmentQuestion[], answers: UserAnswer[], config: AssessmentConfig | QuizConfig): Promise<EvaluationResult> => {
    if (!ai) {
        geminiNotAvailableError('evaluateAssessment');
        return { score: 0, feedback: "Error: Could not evaluate assessment. AI service not available.", detailedResults: [] };
    }
...
export const generateTutorContent = async (topic: string, pace: TutorPace, history: TutorContent[]): Promise<TutorContent> => {
    if (!ai) {
        geminiNotAvailableError('generateTutorContent');
        return { 
            subTopicTitle: "Error",
            explanation: "An error occurred: AI service is not available.",
...
export const executeCode = async (code: string, language: string, input: string): Promise<Pick<CodeExecutionResult, 'output' | 'error'>> => {
    if (!ai) {
        geminiNotAvailableError('executeCode');
        return { output: '', error: 'Failed to execute code: AI service not available.' };
    }
...
export const evaluateCodeAgainstTestCases = async (code: string, language: string, testCases: ProgrammingQuestion['testCases']): Promise<CodeExecutionResult> => {
    if (!ai) {
        geminiNotAvailableError('evaluateCodeAgainstTestCases');
        return { success: false, error: 'Failed to judge code: AI service not available.', testResults: [] };
    }
...
export const getConceptExplanation = async (concept: string): Promise<string> => {
    if (!ai) {
        geminiNotAvailableError('getConceptExplanation');
        return "Could not load explanation: AI service not available.";
    }
...
export const generateChallengeQuestion = async (language: string): Promise<ChallengeQuestion> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateChallengeQuestion'));
    }
...
export const evaluateChallengeSubmission = async (code: string, language: string, question: ChallengeQuestion): Promise<ChallengeEvaluationResult> => {
    if (!ai) {
        geminiNotAvailableError('evaluateChallengeSubmission');
        const testCaseResults = await evaluateCodeAgainstTestCases(code, language, question.testCases);
        return {
            testCaseResults,
            logicalThinkingAnalysis: "Could not analyze logic: AI service is not available."
        }
    }
    const testCaseResults = await evaluateCodeAgainstTestCases(code, language, question.testCases);
...
export const generateDailyMission = async (dateString: string, language: string): Promise<ProgrammingQuestion> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateDailyMission'));
    }
...
export const generateStarterCodeForMission = async (question: Omit<ProgrammingQuestion, 'type' | 'starterCode'>, language: string): Promise<string> => {
    if (!ai) {
        geminiNotAvailableError('generateStarterCodeForMission');
        return `// Could not generate starter code for ${language}: AI service not available.`;
    }
...
export const generateLearningPathFromResume = async (resumeText: string, skills: Skill[]): Promise<Pick<LearningPath, 'title' | 'summary' | 'modules'>> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateLearningPathFromResume'));
    }
...
export const generateRoleBasedRoadmap = async (currentRole: string, dreamRole: string, skills: Skill[]): Promise<Pick<LearningPath, 'title' | 'summary' | 'modules'>> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateRoleBasedRoadmap'));
    }
    const skillsString = skills.map(s => `${s.name} (${s.level})`).join(', ');
    const prompt = `
...
export const generateModuleDetails = async (moduleTitle: string, moduleDescription: string): Promise<ModuleDetails> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateModuleDetails'));
    }
    const prompt = `
...
export const generateUserProgressInsights = async (user: UserProfile): Promise<ProgressInsights> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateUserProgressInsights'));
    }
...
export const evaluateSolution = async (question: ProgrammingQuestion, code: string, language: string): Promise<SolutionEvaluation> => {
    if (!ai) {
        geminiNotAvailableError('evaluateSolution');
        const testCaseResults = await evaluateCodeAgainstTestCases(code, language, question.testCases);
        return {
            ...
            timeComplexity: "N/A",
            spaceComplexity: "N/A",
            feedback: "An error occurred during AI analysis: AI service not available.",
        };
    }
...
export const generateConceptQuestion = async (conceptTitle: string): Promise<Omit<ConceptQuestion, 'id'>> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('generateConceptQuestion'));
    }
    const prompt = `
...
export const extractSkillsFromResumeText = async (resumeText: string): Promise<string[]> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('extractSkillsFromResumeText'));
    }
...
export const analyzeResumeText = async (resumeText: string): Promise<ResumeAnalysis> => {
    if (!ai) {
        throw new Error(geminiNotAvailableError('analyzeResumeText'));
    }
